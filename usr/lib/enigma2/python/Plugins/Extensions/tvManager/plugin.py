...
class tvManager(Screen):
    ...
    def keyNumberGlobal(self, number):
        """Number key handling with shortcuts for quick access"""
        print("Pressed number:", number)

        try:
            number = int(number)
            # Number key mapping for quick access
            key_actions = {
                0: self.messagekd,
            ...
            if number in key_actions:
                ...
                print("No action defined for number: %d" % number)

        except ValueError:
            print("Invalid number pressed: %s" % str(number))
        except Exception as e:
            print("Error in keyNumberGlobal: %s" % str(e))
            self.session.open(MessageBox,
                              _("Error executing shortcut: %s") % str(e),
                              MessageBox.TYPE_ERROR,
                              timeout=3)
    ...
    def cleanup_system(self):
        """System cleanup"""
        self.session.openWithCallback(self._execute_cleanup, MessageBox,
                                      _("Cleanup temporary files and cache?"),
                                      MessageBox.TYPE_YESNO)

    def _execute_cleanup(self, answer):
