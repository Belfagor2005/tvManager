...
#                                                                             #
#     Version: 3.3 Professional Edition      Build Date: 2025-02-27           #
...
class tvManager(Screen):
    ...
    def update_oscam_codes(self):
        """Executes the OSCam codes update script with enhanced error handling"""
        ...
        if not exists(script_path):
            self.session.open(
                MessageBox,
                _("Update script not found!\nPath: %s") % script_path,
                MessageBox.TYPE_ERROR, timeout=5
            )
            return
        
        # Check if script is executable
        if not access(script_path, X_OK):
            try:
                chmod(script_path, 0o755)
                self.session.open(
                    MessageBox,
                    _("Script made executable automatically."),
                    MessageBox.TYPE_INFO, timeout=2
                )
            except Exception as e:
                self.session.open(
                    MessageBox,
                    _("Unable to make script executable: %s") % str(e),
                    MessageBox.TYPE_ERROR, timeout=5
                )
                return
        ...
        self.session.openWithCallback(
            lambda result: self._execute_oscam_update(result, script_path),
            MessageBox,
            _("Execute OSCam codes update?\n\nScript: %s") % basename(script_path),
            MessageBox.TYPE_YESNO
        )

    def _execute_oscam_update(self, confirm, script_path):
        """Execute the update after confirmation"""
        if not confirm:
            return
        ...
        title = _("Updating OSCam codes...")
        cmd = "%s > %s 2>&1" % (script_path, log_file)
        ...
        self.session.open(
            MessageBox,
            _("OSCam codes update completed!\nLog saved to: %s") % log_file,
