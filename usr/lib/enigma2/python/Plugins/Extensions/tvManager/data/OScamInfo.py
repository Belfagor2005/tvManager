...
class OSCamInfo(Screen):
    ...
    def __init__(self, session):
        ...
        self["actions"] = NumberActionMap(
            # Azioni associate (tasti per OK, Input, Colori)
...
class oscECMInfo(Screen, OscamInfo):
    ...
    def __init__(self, session):
        ...
        # Imposta le azioni per i tasti 'ok' e 'cancel' nell'ambito di
        # 'SetupActions'.
    ...
    def buildListEntry(self, listentry):
        return [
            ...
            (eListboxPythonMultiContent.TYPE_TEXT, 10 * f, 2 * f, 300 * f,
             30 * f, 0, RT_HALIGN_LEFT, listentry[0]),  # Primo campo (Testo)
            (eListboxPythonMultiContent.TYPE_TEXT, 300 * f, 2 * f, 300 * f,
             30 * f, 0, RT_HALIGN_LEFT, listentry[1])   # Secondo campo (Testo)
        ]
...
class oscInfo(Screen, OscamInfo):
    ...
    def showData(self):
        ...
        if data[0]:
            if self.what != "l":
                # Create the heading tuple with values from the self.HEAD
                # dictionary
                heading = (
                    self.HEAD[self.NAME],
                    self.HEAD[self.PROT],
    ...
    def __init__(self, session, msg=None):
        ...
        self["actions"] = ActionMap(
            ["SetupActions"],  # The action map group
            {
                ...
                # Action for 'Cancel' key (calls exit method)
                "cancel": self.exit,
            },
