...
update_tivusat_keys() {
    ...
    awk -v start="$start_line" -v end="$end_line" -v new_keys="'$CURRENT_KEYS'" '
        BEGIN {
            # Split the new keys into an array
    ...
    if grep -q "rsakey.*=.*A22DE56" "${TEMP_FILE}_updated"; then
...
# Main script
main() {
    log "Starting OSCam Tivusat Complete Keys Update Script..."
    ...
    if [ -n "$tivusat_line" ]; then
        ...
        if update_tivusat_keys "$tivusat_line"; then
