...
def _parse(url):
    ...
    if ':' in host:
        ...
        port = int(port)
    print("[CCcamInfo]2 parsed=%s scheme=%s path=%s host=%s port=%s" %
          (parsed, scheme, pathz, host, port))
...
def getPage(url, callback, errback):
    ...
    try:
        if 'username' in auth_headers and 'password' in auth_headers:
            credentials = "%s:%s" % (
                auth_headers['username'], auth_headers['password'])
...
def translateBlock(block):
    # Assumiamo che TranslationHelper sia una lista di tuple (coppie
    # chiave-valore)
...
def notBlackListed(entry):
    try:
        ...
        # In caso di errore di lettura del file, logga l'errore e considera
        # tutti gli entry come non blacklisted
...
def getConfigNameAndContent(fileName):
    ...
    if content.startswith("#CONFIGFILE NAME="):
        ...
        # Rimuovi "/etc/" dal percorso del file
...
def CCcamConfigListEntry(file):
    ...
    try:
        with open(CFG, "r") as f:
            org = f.read()
        # Check if the content is bytes (Python 2 compatibility)
...
class CCcamInfoMain(Screen):
    ...
    def readConfig(self):
        ...
        try:
            ...
            return
        # Se username e password sono presenti, aggiorna l'URL con le
        # credenziali
                ...
                if first_line:
                    ...
                    if len(fields) > 7:
                        ...
                        if hostname:
                            ...
                            # Elimina spazi bianchi e separa i valori di uphops
                            # e maxdown
...
class CCcamInfoSubMenu(Screen):
    def __init__(self, session, list, infoList, set_title):
        ...
        self["actions"] = ActionMap(
            ["CCcamInfoActions"], {
                "ok": self.okClicked, "cancel": self.close, "red": self.close, "green": self.okClicked}, -1)
...
class CCcamInfoProfileSetup(Setup):
    def __init__(self, session, profile):
        ...
        self["actions"] = ActionMap(
            ["CCcamInfoActions"], {
                "ok": self.okClicked, "cancel": self.close}, -2)
...
def Plugins(**kwargs):
    lst = [
        PluginDescriptor(
            name=_("CCcam Info %s") %
            VERSION,
            description=_("This plugin shows you the status of your CCcam"),
