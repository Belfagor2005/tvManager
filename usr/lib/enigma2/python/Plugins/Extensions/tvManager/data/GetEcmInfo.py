            ...
            if using:
                ...
                if using == 'fta':
                    ...
                    if hops and hops != '0':
                        ...
                        hops = ''
                    self.textvalue = info.get(
                        'address', '?') + hops + " (%ss)" % info.get('ecm time', '?')
            else:
                ...
                if decode:
                    ...
                    if info['response'] and 'CaID 0x' in ecm[0] and 'pid 0x' in ecm[0]:
                        ...
                        info['caid'] = ecm[0][ecm[0].find(
                            'CaID 0x') + 7:ecm[0].find(',')]
                        info['pid'] = ecm[0][ecm[0].find(
                            'pid 0x') + 6:ecm[0].find(' =')]
                    ...
                    if source:
                        ...
                        if reader:
                            ...
                            if hops and hops != '0':
                                ...
                                hops = ''
                            self.textvalue = reader + hops + \
                                " (%ss)" % info.get('ecm time', '?')
                        else:
                            ...
                            if response:
                                ...
                                self.textvalue = "%s (%ss)" % (
                                    response[4], float(response[0]) / 1000)
                            else:
                                self.textvalue = ""
